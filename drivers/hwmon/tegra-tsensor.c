		...
		unsigned int *p_counter2)
{
	...
	To calculate root - assume
		b = A * (2 * m * B + n)
		a = m * A^2+		c = ((m * B^2) + n * B + p - temp)
	root1 = (-b + sqrt(b^2 - (4*a*c))) / (2 * a)
	root2 = (-b - sqrt(b^2 - (4*a*c))) / (2 * a)
	sqrt(k) = sqrt(k * 10^6) / sqrt(10^6)++	Roots are :
	...
	struct tegra_tsensor_data *data)
{
	...
	/* We want to take num_readings counter values in between+	and try to report corresponding temperature */
